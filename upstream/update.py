...
log = logging.getLogger("updating keystone-k8s-auth")
...
def download(source: str, release: Release) -> Release:
    ...
    for idx, manifest in enumerate(release.paths):
        ...
        patch = FILEDIR / source / "patches" / (Path(manifest).name + ".patch")
        dest.parent.mkdir(exist_ok=True)
        log.info(f"Fetching {release.name} from {manifest}")
        urllib.request.urlretrieve(manifest, dest)
        if patch.exists():
            log.info("Applying patch to %s", dest)
            subprocess.check_call(["patch", "-u", dest, "-i", patch])
        paths.append(dest)
